...
if (!auth.includes('clearSession();') || !auth.includes('currentUser.clear();')) {
  failures.push('invalid sessions do not fully clear client auth state');
}

if (!routes.includes('canActivate: [authGuard]')) {
...
for (const file of await walk('src/app')) {
  if (file === 'src/app/core/config/api.config.ts') {
    continue;
  }

  const source = await readFile(file, 'utf8');

  if (/https?:\/\/localhost:8080/.test(source)) {
    failures.push(`hardcoded local API endpoint: ${file}`);
  }
}
