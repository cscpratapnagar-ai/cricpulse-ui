...
@Component({
    ...
    <div class="field" [class.disabled]="disabled">
      ...
      <div class="dropdown">
        <button type="button" class="trigger" [class.open]="open" [disabled]="disabled"
          ...
          <span>{{ selectedLabel }}</span><i class="chevron" aria-hidden="true"></i>
        </button>
        @if (open) {
  ...
  styles: [`
    ...
    .trigger:hover,.trigger.open{border-color:color-mix(in srgb,var(--cp-accent) 55%,var(--cp-border));box-shadow:0 0 0 3px var(--cp-accent-soft)}.chevron{width:7px;height:7px;flex:0 0 7px;border-right:1.8px solid var(--cp-text-muted);border-bottom:1.8px solid var(--cp-text-muted);transform:rotate(45deg) translate(-1px,-1px);transition:transform .18s ease,border-color .18s ease}.trigger.open .chevron{transform:rotate(225deg) translate(-1px,-1px);border-color:var(--cp-accent)}
