    ...
    <main class="public-live">
      ...
      @if (score$ | async; as score) {
        ...
        <section class="info-grid">
          ...
          <article><span>STATUS</span><strong>{{ score.status || 'LIVE' }}</strong></article>
        </section>
        ...
        @if (score.partnership) {
          <section class="info-grid partnership-grid">
            ...
            <article><span>PARTNERSHIP BALLS</span><strong>{{ score.partnership.balls }}</strong></article>
        ...
        <section class="score-card loading">
          <div>
            <span>Loading saved live score…</span>
            <small>Waiting for the current innings</small>
          </div>
        </section>
      }
...
export class PublicLiveScoreComponent {
  ...
  constructor() {
    ...
    this.http.get<Match>(`${this.api}/matches/${this.matchId}`).subscribe({
      ...
      error: error => console.error('[PublicLive] match load failed', error)
    });

    // Use the same persisted current-innings endpoint as the Resume Match flow.
    // Do not depend on a WebSocket event to obtain the initial score.
    this.http.get<CurrentInnings>(`${this.api}/matches/${this.matchId}/current-innings`).subscribe({
      next: innings => {
        const inningsId = innings.id || innings.inningsId;
        if (!inningsId) {
          console.error('[PublicLive] current innings response has no id', innings);
          return;
        }

        this.score$ = this.liveScore.watch(inningsId).pipe(
          catchError(error => {
            console.error('[PublicLive] live score failed', error);
            return of<LiveScore | null>(null);
          })
        );
      },
      error: error => {
        console.error('[PublicLive] current innings load failed', error);
        this.score$ = of(null);
      }
    });
  }
  ...
  runRate(score: LiveScore): string {
    return score.legalBalls
      ? (score.runs / (score.legalBalls / 6)).toFixed(2)
      : '0.00';
  }
}
