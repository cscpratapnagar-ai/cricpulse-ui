              ...
              <div class="selection-board">
                <section class="squad-list">
                  <div class="panel-head">
                    <div><span>Squad</span><h3>Available players</h3><p>{{team.members.length}} registered · Choose up to 11</p></div>
                    <div class="panel-stat"><b>{{team.members.length}}</b><small>Players</small></div>
                  </div>
                  <div class="players-scroll player-grid">
                    @for (m of team.members; track m.playerId) {
                      <article class="player-card" [class.selected]="isSelected(team,m.playerId)" (click)="match.status==='SCHEDULED' && toggle(team,m)">
                        <div class="player-card-top"><div class="avatar">{{initials(m.fullName)}}</div><div class="check-mark" [class.active]="isSelected(team,m.playerId)">{{isSelected(team,m.playerId)?'✓':'+'}}</div></div>
                        <b [title]="m.fullName">{{m.fullName}}</b>
                        <small>{{m.role || 'Player'}}</small>
                        <button type="button" [disabled]="match.status!=='SCHEDULED' || (!isSelected(team,m.playerId) && team.selected.length>=11)" (click)="$event.stopPropagation(); toggle(team,m)">{{isSelected(team,m.playerId)?'Selected':'Add to XI'}}</button>
                      </article>
                ...
                <section class="xi-list">
                  <div class="xi-top">
                    <div><span>Playing XI</span><h3>{{team.selected.length === 11 ? 'Lineup ready' : 'Build your lineup'}}</h3></div>
                  ...
                  <div class="xi-scroll">
                    @for (p of team.selected; track p.playerId; let i=$index) {
                      <article class="xi-row">
                        ...
                        <div class="player-copy"><b [title]="p.name">{{p.name}}</b><div class="tags">@if(p.captain){<span>C</span>} @if(p.viceCaptain){<span>VC</span>} @if(p.wicketKeeper){<span>WK</span>} @if(!p.captain&&!p.viceCaptain&&!p.wicketKeeper){<small>Playing XI</small>}</div></div>
                        <button [disabled]="match.status!=='SCHEDULED'" (click)="remove(team,p.playerId)" title="Remove player">×</button>
                      </article>
                    } @empty { <div class="xi-empty"><div class="xi-orbit">XI</div><b>Build the Playing XI</b><span>Select players from the squad. Your chosen players will appear here in match order.</span></div> }
                  </div>
                  @if(team.selected.length>0 && team.selected.length<11){<div class="slots-note"><i></i>{{11-team.selected.length}} more {{11-team.selected.length===1?'player':'players'}} needed</div>}
                </section>
              </div>
            }
          </section>
...
.progress{display:grid;grid-template-columns:185px 1fr 120px;align-items:center;gap:18px;margin:16px 0;padding:14px 17px;border:1px solid var(--line);border-radius:15px;background:var(--surface)}.progress-copy{display:grid;gap:4px}.progress-copy span,.progress-meta{color:var(--muted);font-size:14px;font-weight:800;letter-spacing:.65px}.progress-copy b{font-size:14px}.progress-copy small{color:var(--muted);font-size:9px}.progress-track{height:5px;overflow:hidden;border-radius:99px;background:color-mix(in srgb,var(--cp-text) 7%,transparent)}.progress-track i{display:block;height:100%;min-width:2px;border-radius:inherit;background:var(--accent);box-shadow:0 0 12px color-mix(in srgb,var(--accent) 60%,transparent);transition:width .35s ease}.progress-meta{display:flex;justify-content:space-between}.progress-meta span:last-child{color:var(--accent)}
...
.selection-board{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(350px,.85fr);min-height:540px;background:linear-gradient(90deg,color-mix(in srgb,var(--cp-text) 1%,transparent),transparent)}
.squad-list,.xi-list{min-width:0;padding:24px}.xi-list{border-left:1px solid var(--line);background:color-mix(in srgb,var(--cp-text) 1.8%,transparent)}
.panel-head,.xi-top{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:18px}.panel-head span,.xi-top span{display:block;color:var(--muted);font-size:11px;font-weight:800;letter-spacing:.5px}.panel-head h3,.xi-top h3{margin:5px 0 4px;font-size:18px;letter-spacing:-.4px}.panel-head p{margin:0;color:var(--muted);font-size:12px}.panel-stat{min-width:58px;padding:9px 8px;border:1px solid var(--line);border-radius:12px;text-align:center;background:var(--raised)}.panel-stat b{display:block;font-size:18px}.panel-stat small{display:block;margin-top:2px;color:var(--muted);font-size:9px}
.players-scroll,.xi-scroll{max-height:470px;overflow:auto;padding-right:5px}.player-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;align-content:start}.player-card{position:relative;min-width:0;padding:14px;border:1px solid var(--line);border-radius:15px;background:var(--surface);cursor:pointer;transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease}.player-card:hover{transform:translateY(-2px);border-color:var(--strong);box-shadow:0 12px 26px color-mix(in srgb,#000 7%,transparent)}.player-card.selected{border-color:color-mix(in srgb,var(--accent) 48%,var(--line));background:linear-gradient(145deg,var(--accent-soft),color-mix(in srgb,var(--accent-soft) 35%,var(--surface)))}.player-card-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}.avatar,.xi-avatar{width:38px;height:38px;display:grid;place-items:center;border-radius:12px;background:color-mix(in srgb,var(--accent) 10%,var(--raised));border:1px solid color-mix(in srgb,var(--accent) 18%,var(--line));color:var(--accent);font-size:11px;font-weight:900}.check-mark{width:26px;height:26px;display:grid;place-items:center;border:1px solid var(--line);border-radius:9px;color:var(--muted);font-size:16px;font-weight:700}.check-mark.active{border-color:var(--accent);background:var(--accent);color:var(--cp-accent-contrast)}.player-card>b{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px}.player-card>small{display:block;margin-top:4px;color:var(--muted);font-size:10px}.player-card>button{width:100%;height:32px;margin-top:13px;border:1px solid var(--line);border-radius:9px;background:transparent;color:var(--ink);font-size:11px;font-weight:800;cursor:pointer;transition:.18s}.player-card:hover>button,.player-card.selected>button{border-color:var(--accent);background:var(--accent);color:var(--cp-accent-contrast)}.player-card>button:disabled{opacity:.48;cursor:not-allowed}
.xi-top{margin-bottom:12px}.xi-count{display:flex;align-items:baseline;gap:3px;padding:8px 10px;border-radius:11px;background:var(--accent-soft);color:var(--accent)}.xi-count b{font-size:20px}.xi-count span{color:var(--accent);font-size:11px}.xi-progress{height:6px;overflow:hidden;margin-bottom:17px;border-radius:999px;background:color-mix(in srgb,var(--cp-text) 7%,transparent)}.xi-progress i{display:block;height:100%;min-width:2px;border-radius:inherit;background:var(--accent);box-shadow:0 0 14px color-mix(in srgb,var(--accent) 55%,transparent);transition:width .25s ease}
.xi-row{display:flex;align-items:center;gap:11px;padding:10px;border:1px solid transparent;border-radius:11px;margin-bottom:5px;transition:.18s}.player-row{background:color-mix(in srgb,var(--cp-text) 2%,transparent);border-color:var(--line)}.player-row:hover{border-color:var(--strong);transform:translateX(1px)}.player-row.selected{background:var(--accent-soft);border-color:color-mix(in srgb,var(--accent) 30%,var(--line))}.avatar{width:36px;height:36px;flex:0 0 36px;display:grid;place-items:center;border-radius:9px;background:var(--raised);color:var(--accent);font-size:13px;font-weight:800}.player-copy{min-width:0;flex:1;display:grid;gap:3px}.player-copy>b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px}.player-copy small{color:var(--muted);font-size:10px}.player-row button{display:inline-flex;align-items:center;justify-content:center;gap:5px;min-width:68px;height:36px;border:0;border-radius:9px;padding:0 9px;background:var(--accent);color:var(--cp-accent-contrast);font-size:14px;font-weight:800;line-height:1;cursor:pointer;white-space:nowrap}.player-row button .action-icon{display:grid;place-items:center;width:11px;height:11px;font-size:13px;font-weight:800;line-height:11px;transform:translateY(-.5px)}.player-row button .action-label{line-height:1}.player-row button.remove{background:transparent;color:var(--accent);border:1px solid color-mix(in srgb,var(--accent) 30%,var(--line))}.player-row button:disabled{opacity:.42;cursor:not-allowed}
.xi-row{display:flex;align-items:center;gap:10px;margin:0 0 7px;padding:9px;border:1px solid var(--line);border-radius:13px;background:var(--surface);transition:.18s}.xi-row:hover{border-color:var(--strong);transform:translateX(2px)}.order{width:24px;flex:0 0 24px;color:var(--muted);font-size:10px;font-weight:900;text-align:center}.xi-avatar{width:32px;height:32px;flex:0 0 32px;border-radius:10px;font-size:9px}.tags{display:flex;align-items:center;gap:4px;min-height:12px}.tags span{padding:3px 6px;border-radius:5px;background:var(--accent-soft);color:var(--accent);font-size:9px;font-weight:900}.tags small{font-size:9px;letter-spacing:.7px}.xi-row>button{width:25px;height:25px;border:0;border-radius:7px;background:transparent;color:var(--muted);font-size:18px;cursor:pointer;transition:.15s}.xi-row>button:hover:not(:disabled){background:color-mix(in srgb,#e9636b 10%,transparent);color:#e9636b}.xi-row>button:disabled{opacity:.4}.xi-empty{min-height:350px;display:grid;place-content:center;justify-items:center;text-align:center;gap:9px;color:var(--muted)}.xi-orbit{width:74px;height:74px;display:grid;place-items:center;border:1px dashed color-mix(in srgb,var(--accent) 45%,var(--line));border-radius:24px;background:radial-gradient(circle,var(--accent-soft),transparent 70%);color:var(--accent);font-size:17px;font-weight:900;box-shadow:0 0 0 12px color-mix(in srgb,var(--accent) 4%,transparent)}.xi-empty b{margin-top:8px;color:var(--ink);font-size:15px}.xi-empty span{max-width:250px;font-size:12px;line-height:1.65}.slots-note{display:flex;align-items:center;gap:7px;margin-top:14px;padding:10px 11px;border-radius:10px;background:var(--accent-soft);color:var(--accent);font-size:11px;font-weight:800}.slots-note i{width:6px;height:6px;border-radius:50%;background:var(--accent);box-shadow:0 0 10px var(--accent)}.empty,.loading{padding:35px 15px;text-align:center;color:var(--muted);font-size:14px}.access-denied{padding:38px 28px;text-align:center}.lock{width:42px;height:42px;margin:0 auto 12px;display:grid;place-items:center;border-radius:13px;background:var(--accent-soft);color:var(--accent);font-size:19px}.access-denied b{font-size:14px}.access-denied p{margin:7px auto 0;max-width:360px;color:var(--muted);font-size:14px;line-height:1.6}.roles{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:14px;border-top:1px solid var(--line)}.roles div{min-width:0;padding:10px;border:1px solid var(--line);border-radius:10px;background:color-mix(in srgb,var(--cp-text) 2%,transparent);display:grid;gap:5px}.roles strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px}
