                    ...
                    @for (m of team.members; track m.playerId) {
                      <article class="player-row" [class.selected]="isSelected(team,m.playerId)">
                        ...
                        <button [class.remove]="isSelected(team,m.playerId)" [disabled]="match.status!=='SCHEDULED' || (!isSelected(team,m.playerId) && team.selected.length>=11)" (click)="toggle(team,m)">
                          <span>{{isSelected(team,m.playerId)?'✓':'+'}}</span>{{isSelected(team,m.playerId)?'Selected':'Add'}}
                        </button>
                      </article>
