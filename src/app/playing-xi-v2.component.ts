        ...
        @for (team of teams; track team.id) {
          <section class="card">
            ...
            @else {
              ...
              <div class="roles"><div><small>Captain</small><strong>{{roleName(team,team.captainId) || 'Not selected'}}</strong></div><div><small>Vice Captain</small><strong>{{roleName(team,team.viceCaptainId) || 'Not selected'}}</strong></div><div><small>Wicket Keeper</small><strong>{{roleName(team,team.wicketKeeperId) || 'Not selected'}}</strong></div></div>
              @if (team.accessible) {
...
export class PlayingXiV2Component {
  ...
  saveRoles(t:TeamState){const calls=t.selected.map(p=>this.http.post(`${this.api}/matches/${this.match!.id}/playing-xi`,{teamId:t.id,playerId:p.playerId,captain:p.playerId===t.captainId,viceCaptain:p.playerId===t.viceCaptainId,wicketKeeper:p.playerId===t.wicketKeeperId}));if(!calls.length){this.roleTeam=null;return;}this.saving=true;let done=0;let failed=false;calls.forEach(c=>c.subscribe({next:()=>{done++;if(done===calls.length&&!failed){this.saving=false;this.roleTeam=null;this.loadXi(t,this.match!.id);this.toast='Captain, vice captain and wicket keeper saved';}},error:e=>{failed=true;this.saving=false;this.toast=e?.error?.message||'Unable to save roles';}}));}
