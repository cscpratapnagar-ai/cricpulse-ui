            ...
            @else {
              ...
              @if (team.accessible) {
                <div class="role-strip">
                  <div class="role-strip-head"><div><b>Match Roles</b><small>Inherited from Team Management when available</small></div><button class="save-role" [disabled]="saving || match.status!=='SCHEDULED' || !team.selected.length" (click)="saveRoles(team)">{{saving?'Saving…':'Save roles'}}</button></div>
                  <div class="role-grid">
                    ...
                    <app-select-field label="Wicket Keeper" name="wicketKeeper" placeholder="Select wicket keeper" [options]="roleOptions(team)" [(value)]="team.wicketKeeperId" />
                  </div>
                </div>
                ...
                </div><div class="xi"><h3>Playing XI</h3>@for (p of team.selected; track p.playerId; let i=$index) { <div class="xi-row"><span>{{i+1}}</span><div class="name"><b>{{p.name}}</b><small>{{p.captain?'Captain':''}}{{p.captain&&p.viceCaptain?' · ':''}}{{p.viceCaptain?'Vice Captain':''}}{{(p.captain||p.viceCaptain)&&p.wicketKeeper?' · ':''}}{{p.wicketKeeper?'Wicket Keeper':''}}</small></div><button [disabled]="match.status!=='SCHEDULED'" (click)="remove(team,p.playerId)">×</button></div> } @empty {<div class="empty">Select up to 11 players.</div>}</div></div>
              } @else { <div class="roles"><div><small>Captain</small><strong>{{roleName(team,team.captainId) || 'Not selected'}}</strong></div><div><small>Vice Captain</small><strong>{{roleName(team,team.viceCaptainId) || 'Not selected'}}</strong></div><div><small>Wicket Keeper</small><strong>{{roleName(team,team.wicketKeeperId) || 'Not selected'}}</strong></div></div> }
            }
          </section>
        }
      </div>
      <div class="bar"><span>Select up to 11 players for each side. Match roles are saved separately.</span><button [disabled]="saving || !canSave" (click)="saveAll()">{{saving?'Saving…':'Save Playing XI'}}</button></div>
      @if(toast){<div class="toast">{{toast}}</div>}
    }
  </section>`,
  styles:[`:host{display:block}.page{max-width:1250px;padding:34px 4vw 120px;color:#edf8f2}.hero{display:flex;justify-content:space-between;gap:20px;align-items:end;margin-bottom:22px}.hero a{display:block;color:#91aa9d;text-decoration:none;font-size:12px;margin-bottom:15px}.hero span,.team-head small,.roles small{color:#b8f45c;font-size:9px;letter-spacing:1.7px;font-weight:800}.hero h1{font-size:clamp(38px,5vw,62px);letter-spacing:-3px;margin:8px 0}.hero p{color:#91aa9d}.hero p b{color:#b8f45c}.status{padding:9px 12px;border-radius:999px;background:#b8f45c12;color:#b8f45c;font-size:10px}.notice{padding:13px 15px;border:1px solid #ffffff15;border-radius:12px;background:#ffffff05;color:#91aa9d;font-size:11px;margin-bottom:15px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:18px}.card{background:#0c2119d9;border:1px solid #ffffff15;border-radius:20px;overflow:visible}.team-head{display:flex;justify-content:space-between;align-items:center;padding:20px;border-bottom:1px solid #ffffff0d}.team-head h2{margin:6px 0 0;font-size:24px}.team-head>b{padding:8px 11px;border-radius:999px;background:#b8f45c12;color:#b8f45c}.role-strip{position:relative;z-index:10;padding:15px;border-bottom:1px solid #ffffff0d;background:#07150f66}.role-strip-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:13px}.role-strip-head>div{display:grid;gap:4px}.role-strip-head b{font-size:12px}.role-strip-head small{font-size:9px;color:#789386}.role-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.save-role{border:1px solid #b8f45c35;background:#b8f45c;color:#10251e;border-radius:8px;padding:8px 11px;font-size:9px;font-weight:850;cursor:pointer}.save-role:disabled{opacity:.45}.roles{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;padding:14px}.roles div{padding:11px;background:#ffffff05;border-radius:11px;display:grid;gap:5px}.roles strong{font-size:12px}.columns{display:grid;grid-template-columns:1fr 1fr;border-top:1px solid #ffffff0d}.columns>div{padding:14px}.xi{border-left:1px solid #ffffff0d;background:#07150f66}.columns h3{font-size:12px;margin:0 0 10px}.columns h3 small{color:#789386;font-weight:400}.player,.xi-row{display:flex;align-items:center;gap:8px;padding:8px;border:1px solid #ffffff0c;border-radius:9px;margin-bottom:6px}.player.selected{border-color:#b8f45c35;background:#b8f45c08}.avatar{width:32px;height:32px;border-radius:8px;background:#173d2d;color:#b8f45c;display:grid;place-items:center;font-weight:900;font-size:10px}.name{flex:1;min-width:0;display:grid;gap:2px}.name b{font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.name small{font-size:9px;color:#789386}.player button,.bar button{border:0;border-radius:8px;padding:7px 9px;background:#b8f45c;color:#10251e;font-size:9px;font-weight:800;cursor:pointer}.player button:disabled,.bar button:disabled{opacity:.4}.xi-row>span{width:20px;color:#789386;font-size:9px}.xi-row button{border:0;background:transparent;color:#91aa9d;font-size:18px;cursor:pointer}.empty{text-align:center;padding:25px;color:#789386;font-size:10px}.bar{position:sticky;bottom:18px;margin-top:18px;padding:14px 16px;background:#07150ff2;border:1px solid #b8f45c25;border-radius:14px;display:flex;justify-content:space-between;align-items:center;gap:15px}.bar span{color:#789386;font-size:10px}.loading{padding:35px;color:#789386}.toast{position:fixed;right:20px;bottom:20px;background:#173d2d;color:#fff;padding:12px 15px;border-radius:10px;z-index:1200}@media(max-width:1000px){.grid{grid-template-columns:1fr}}@media(max-width:700px){.hero,.bar,.role-strip-head{flex-direction:column;align-items:stretch}.role-grid{grid-template-columns:1fr}.columns{grid-template-columns:1fr}.xi{border-left:0;border-top:1px solid #ffffff0d}}`]
})
export class PlayingXiV2Component {
  ...
  loadingMatch=true; saving=false; match:Match|null=null; teams:TeamState[]=[]; toast='';
