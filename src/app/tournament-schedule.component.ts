...
@Component({
 ...
 @if(loading){<section class="card state">Loading schedule…</section>}
 @else if(t){
  <header class="hero card"><div><span class="eyebrow">CRICPULSE · FIXTURE PLANNER</span><h1>{{t.name}}</h1><p>{{t.format}} · {{t.overs}} overs · {{fixtures.length}} fixtures</p></div><span class="badge">SCHEDULE</span></header>
  @if(message){<div class="notice">{{message}}</div>}
  @if(error){<div class="error">{{error}}</div>}
  <section class="card panel"><div class="head"><div><span class="eyebrow">MATCH CALENDAR</span><h2>Schedule fixtures</h2></div><span class="hint">Times use your selected local time</span></div>
   @if(fixtures.length){<div class="fixtures">@for(f of fixtures;track f.matchId){<article class="fixture"><div class="number">#{{f.fixtureNumber||'—'}}</div><div class="match"><span>{{f.stage}}</span><strong>{{f.teamAName}} <b>vs</b> {{f.teamBName}}</strong><small>{{f.matchName}} · {{f.status}}</small></div><div class="schedule"><app-date-time-field label="Match date & time" [includeTime]="true" [value]="draft[f.matchId]||''" (valueChange)="draft[f.matchId]=$event"></app-date-time-field><button class="primary" [disabled]="busy===f.matchId||!draft[f.matchId]" (click)="save(f)">{{busy===f.matchId?'Saving…':f.scheduledAt?'Reschedule':'Schedule'}}</button><small class="current">{{f.scheduledAt?'Current: '+format(f.scheduledAt):'Not scheduled'}}</small></div></article>}</div>}
   @else{<div class="empty">No fixtures available. Generate league fixtures first.</div>}
  </section>
 } @else {<section class="card state"><h2>Tournament not found</h2><a routerLink="/tournaments">Back to tournaments →</a></section>}
 </section>`,
