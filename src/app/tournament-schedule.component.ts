...
<section class="page">
  ...
  @if (loading) {
          ...
          @for (f of fixtures; track f.matchId) {
            <article class="fixture">
              ...
              <div class="schedule">
                ...
                <button class="primary" [disabled]="busy===f.matchId || !draft[f.matchId]" (click)="save(f)">{{busy===f.matchId ? 'Saving…' : f.scheduledAt ? 'Reschedule' : 'Schedule'}}</button>
    ...
    <section class="card state"><h2>Tournament not found</h2><a routerLink="/tournaments">Back to tournaments →</a></section>
  }
</section>`,
