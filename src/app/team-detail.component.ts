...
<section class="team-page">
  @if(loading){
    ...
    <nav class="tabs" aria-label="Team sections">
      ...
      <button [class.active]="tab==='squad'" (click)="setTab('squad')">Squad <span>{{members.length}}</span></button>
      @if(access?.canManage){<button [class.active]="tab==='management'" (click)="setTab('management')">Management</button>}
    </nav>
    ...
    @if(tab==='overview'){
      <section class="overview-grid">
        <article class="panel squad-health">
          ...
          <div class="composition">
            <div class="ring"><strong>{{squadHealth}}%</strong><small>READY</small></div>
            <div class="roles">
      ...
      <section class="panel roster-preview">
        <div class="panel-head"><div><div class="eyebrow"><span></span> ROSTER</div><h2>Squad preview</h2></div><button class="text-btn" (click)="setTab('squad')">View full squad →</button></div>
      ...
      <section class="panel squad-panel">
        ...
        @if(showAdd && access?.canManage){
          <div class="add-drawer">
            ...
            <button class="primary" (click)="addMember()" [disabled]="saving">Add player</button>
            <button class="icon-btn" (click)="showAdd=false" aria-label="Close">×</button>
          </div>
        }
            ...
            <article class="member-row">
              ...
              @if(access?.canManage){
                <div class="member-actions">
                  ...
                  <button class="remove" [disabled]="m.role==='OWNER'||saving" (click)="requestRemove(m)">Remove</button>
                </div>
              }
            </article>
          } @empty {
    ...
    @if(tab==='management' && access?.canManage){
      <section class="management-grid">
        ...
        <article class="panel danger-zone"><div class="eyebrow"><span></span> SAFETY</div><h2>Roster changes are live</h2><p>Role updates and removals immediately update the team workspace.</p><button class="text-btn" (click)="setTab('squad')">Manage squad →</button></article>
      </section>
    }

    @if(confirmMember){
      <div class="modal-backdrop" (click)="confirmMember=null">
        <section class="confirm-card" (click)="$event.stopPropagation()"><div class="confirm-icon">!</div><div><div class="eyebrow"><span></span> CONFIRM CHANGE</div><h2>Remove {{confirmMember.fullName}}?</h2><p>This player will lose access to this team roster. You can add them again later.</p></div><div class="confirm-actions"><button class="ghost" (click)="confirmMember=null">Cancel</button><button class="danger" (click)="removeConfirmed()">Remove player</button></div></section>
      </div>
    }
