...
export class CreateMatchComponent {
  ...
  submit(): void {
    this.error = '';
    if (!this.form.name.trim() || !this.form.teamAId || !this.form.teamBId) {
      this.error = 'Complete the match name and select both teams.';
      return;
    }
    if (this.form.teamAId === this.form.teamBId) {
      this.error = 'Your team and opponent must be different teams.';
      return;
    }
    this.loading = true;
    this.http
      ...
      .subscribe({
        ...
        error: () => {
          this.loading = false;
          this.error = 'Match could not be created. Check the selected teams and try again.';
        },
      });
  }
}
