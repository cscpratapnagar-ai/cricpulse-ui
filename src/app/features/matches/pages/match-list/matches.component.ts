  ...
  template: `
    <section class="matches-page">
      ...
      @if(loading){
        ...
        <app-state-view state="error" title="Couldn’t load your matches" message="The workspace connection needs another try." (retry)="load()"></app-state-view>
        ...
        <app-state-view state="empty" [title]="query ? 'No matches found' : 'No matches in view'" [message]="query ? 'Try a different search or switch filters.' : 'Your matchday workspace is ready for its first fixture.'" [actionLabel]="query ? 'Clear search' : 'Create your first match'" (action)="handleEmptyAction()"></app-state-view>
      }
    </section>
  `,
  styles: [`
    ...
    .matches-page{position:relative;isolation:isolate;max-width:1440px;margin:0 auto;padding:44px 4vw 100px;color:var(--ink);overflow:hidden;animation:pageIn .5s cubic-bezier(.22,1,.36,1) both}
    ...
    @keyframes enter{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:none}}@keyframes pageIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}@keyframes livePulse{50%{opacity:.42}}@keyframes shimmer{to{background-position:-200% 0}}
