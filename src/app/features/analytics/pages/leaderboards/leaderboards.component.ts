...
<section class="leaderboards-page">
  ...
  @if(loading){
    ...
    <nav class="board-tabs" role="tablist" aria-label="Leaderboard categories">
      @for(tab of tabs;track tab.key){
        <button type="button" [class.active]="active===tab.key" (click)="setBoard(tab.key)" role="tab" [attr.aria-selected]="active===tab.key">
...
.board-tabs{position:sticky;top:12px;z-index:10;display:flex;gap:6px;width:max-content;max-width:100%;margin:16px 0;padding:6px;border:1px solid var(--cp-border);border-radius:14px;background:color-mix(in srgb,var(--cp-surface) 88%,transparent);backdrop-filter:blur(14px);box-shadow:var(--cp-shadow-sm);overflow:auto}.board-tabs button{appearance:none;border:0;padding:10px 14px;border-radius:9px;background:transparent;color:var(--cp-text-muted);font:inherit;font-size:12px;font-weight:850;white-space:nowrap;cursor:pointer;transition:.18s}.board-tabs button span{margin-right:7px;color:var(--cp-accent);transition:.18s}.board-tabs button:hover{background:var(--cp-accent-soft);color:var(--cp-accent)}.board-tabs button.active{background:var(--cp-accent);color:var(--cp-accent-contrast);box-shadow:0 5px 14px color-mix(in srgb,var(--cp-accent) 24%,transparent)}.board-tabs button.active span{color:inherit}
