...
import { LiveRecentBall, LiveScore, LiveScoreService } from '../../../../core/services/live-score.service';
...
export class BroadcastOverlayComponent {
  ...
  private detectEvent(score: LiveScore, previous: LiveScore | null): AutoEvent | null {
    ...
    if (score.status === 'COMPLETED' && previous?.status !== 'COMPLETED') return this.makeEvent('RESULT', 'MATCH COMPLETE', `${score.runs}/${score.wickets}`, '✓', score);
    if (ball.wicketType) return this.makeEvent('WICKET', 'WICKET', 'WICKET FALLEN', 'W', score);
    ...
    if (ball.legalDelivery && score.legalBalls > 0 && score.legalBalls % 6 === 0) return this.makeEvent('OVER_COMPLETE', 'OVER COMPLETE', `${this.overs(score.legalBalls)} OVERS`, 'OV', score);
    return null;
  }
  ...
  private isNewState(score: LiveScore, previous: LiveScore | null) {
    ...
    return score.runs !== previous.runs || score.wickets !== previous.wickets || score.legalBalls !== previous.legalBalls;
  }

  private isMilestone(score: LiveScore, previous: LiveScore | null) {
    if (!previous) return false;
    return [25, 50, 75, 100, 125, 150, 175, 200, 250, 300].some((value) => previous.runs < value && score.runs >= value);
  }
