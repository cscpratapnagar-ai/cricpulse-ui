...
interface Qualifier { seed:number; label:string; higherSeed:Point; lowerSeed:Point; }
interface Preview { eligible:boolean; message:string; table:Point[]; qualifiers:Qualifier[]; }
...
export class TournamentQualificationComponent {
 ...
 id=this.route.snapshot.paramMap.get('id')||''; loading=true; preview:Preview|null=null; error='';
 constructor(){if(this.id)this.load();else this.loading=false;}
 load(){this.http.get<Preview>(`${this.api}/tournaments/${this.id}/qualification`).subscribe({next:r=>{this.preview=r;this.loading=false;},error:e=>{this.error=e?.error?.message||'Unable to load qualification preview.';this.loading=false;}});}
}
