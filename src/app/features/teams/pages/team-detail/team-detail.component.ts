...
export class TeamDetailComponent {
  ...
  loading=true; loadError=false; tab:Tab='overview'; query=''; roleFilter='ALL'; roleFilterOpen=false; email=''; role='PLAYER'; addRoleOpen=false; memberRoleOpen:string|null=null; showAdd=false; saving=false;
  readonly roleFilterOptions=[{value:'ALL',label:'All roles',icon:'◉'},{value:'OWNER',label:'Owner',icon:'◆'},{value:'MANAGER',label:'Manager',icon:'◈'},{value:'CAPTAIN',label:'Captain',icon:'★'},{value:'VICE_CAPTAIN',label:'Vice captain',icon:'✦'},{value:'PLAYER',label:'Player',icon:'●'}];
  readonly addRoleOptions=[{value:'PLAYER',label:'Player',icon:'●'},{value:'CAPTAIN',label:'Captain',icon:'★'},{value:'VICE_CAPTAIN',label:'Vice captain',icon:'✦'},{value:'MANAGER',label:'Manager',icon:'◈'}];
  readonly memberRoleOptions=[{value:'OWNER',label:'Owner',icon:'◆'},{value:'MANAGER',label:'Manager',icon:'◈'},{value:'CAPTAIN',label:'Captain',icon:'★'},{value:'VICE_CAPTAIN',label:'Vice captain',icon:'✦'},{value:'PLAYER',label:'Player',icon:'●'}];
  ...
  setTab(tab:Tab){this.tab=tab;this.closeDropdowns();}
  closeDropdowns(){this.roleFilterOpen=false;this.addRoleOpen=false;this.memberRoleOpen=null;}
  toggleRoleFilter(){const next=!this.roleFilterOpen;this.closeDropdowns();this.roleFilterOpen=next;}
  toggleAddRole(){const next=!this.addRoleOpen;this.closeDropdowns();this.addRoleOpen=next;}
  toggleMemberRole(playerId:string){const next=this.memberRoleOpen===playerId?null:playerId;this.closeDropdowns();this.memberRoleOpen=next;}
  get roleFilterLabel(){return this.roleFilterOptions.find(x=>x.value===this.roleFilter)?.label||'All roles';}
  get addRoleLabel(){return this.addRoleOptions.find(x=>x.value===this.role)?.label||'Player';}
  roleLabel(value:string){return this.memberRoleOptions.find(x=>x.value===value)?.label||value.replace('_',' ');}
  setRoleFilter(value:string){this.roleFilter=value;this.closeDropdowns();}
  setAddRole(value:string){this.role=value;this.closeDropdowns();}
  selectMemberRole(member:Member,value:string){this.closeDropdowns();if(value!==member.role)this.changeRole(member,value);}
