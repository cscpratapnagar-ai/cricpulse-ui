...
export class PlayerOnboardingComponent {
  ...
  constructor() {
    const savedUser = localStorage.getItem('cricketpulse_user');
    const savedTeam = localStorage.getItem('cricketpulse_team');
    this.user = savedUser ? (JSON.parse(savedUser) as User) : null;
    this.team = savedTeam ? (JSON.parse(savedTeam) as { id: string; name: string }) : null;
    if (!this.user) {
  ...
  private loadProfile(): void {
    this.http.get<Player>('http://localhost:8080/api/players/me').subscribe({
      next: (player) => {
        ...
        this.playingRole = player.playingRole || 'ALL_ROUNDER';
      ...
      error: (response) => {
        if (response.status !== 404) this.error = 'Could not load your player profile.';
      },
    });
  }
  save(): void {
    ...
    this.error = '';
    if (this.jerseyNumber !== null && (this.jerseyNumber < 0 || this.jerseyNumber > 99)) {
      this.error = 'Jersey number must be between 0 and 99.';
      return;
    }
    if (this.bio.length > 500) {
      this.error = 'Bio must be 500 characters or less.';
      return;
    }
    this.loading = true;
    const payload = {
  ...
  private apiError(response: any): string {
    ...
    return 'Could not save your player profile. Please try again.';
  }
}
