...
export class PlayerComparisonComponent implements OnInit{
 ...
 get leftOptions():SelectOption[]{return this.players.filter(p=>p.id!==this.rightId).map(p=>({value:p.id,label:p.name}));}
