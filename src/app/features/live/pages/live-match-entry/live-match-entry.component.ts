...
@Component({
  ...
  templateUrl: './live-match-entry.component.html',
  styleUrl: './live-match-entry.component.scss',
})
export class LiveMatchEntryComponent {
  ...
  check(): void {
    ...
    this.http.get<MatchState>(`${this.api}/matches/${this.matchId}`).subscribe({
      next: (match) => {
  ...
  resume(): void {
    ...
    if (!id) {
      this.error = 'Existing innings id is missing from the server response.';
      return;
    }
    void this.router.navigateByUrl(
      '/matches/' + this.matchId + '/live-scoring?inningsId=' + encodeURIComponent(id),
    );
  }
  openResult(): void {
    void this.router.navigateByUrl('/matches/' + this.matchId + '/result');
  }
  retry(): void {
    this.loading = true;
    this.error = '';
