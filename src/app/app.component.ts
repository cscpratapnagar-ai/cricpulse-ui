...
@Component({
  ...
  styles: [`
    ...
    .loader-overlay{position:fixed;inset:0;z-index:1000;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at top,#133c2d 0,rgba(7,19,15,.86) 38%,rgba(3,8,6,.96) 100%);backdrop-filter:blur(10px)}
    .loader-card{position:relative;width:min(520px,calc(100vw - 32px));padding:28px 26px 24px;border:1px solid rgba(184,244,92,.18);border-radius:26px;background:linear-gradient(180deg,rgba(12,33,25,.95),rgba(8,20,15,.96));box-shadow:0 24px 80px rgba(0,0,0,.45);overflow:hidden}
    ...
    .brand-mark{display:grid;place-items:center;width:42px;height:42px;border-radius:14px;background:#b8f45c;color:#10251e;font-size:18px;font-weight:900;box-shadow:0 0 0 6px rgba(184,244,92,.09)}
    .brand-row b{display:block;font-size:14px;letter-spacing:.4px}
    .brand-row small{display:block;margin-top:2px;color:#91aa9d;font-size:11px}
    .pitch-loader{position:relative;height:132px;margin:6px 0 18px;border-radius:22px;border:1px solid rgba(255,255,255,.08);background:linear-gradient(180deg,rgba(255,255,255,.05),rgba(255,255,255,.02))}
    .pitch-loader:before{content:'';position:absolute;left:18px;right:18px;top:50%;height:2px;transform:translateY(-50%);background:linear-gradient(90deg,transparent,#b8f45c55,#b8f45c,#b8f45c55,transparent);box-shadow:0 0 26px rgba(184,244,92,.28)}
    .stump{position:absolute;bottom:28px;width:8px;height:52px;border-radius:999px;background:linear-gradient(180deg,#d9ffd1,#b8f45c 60%,#6f9428)}
    .stump:before{content:'';position:absolute;left:50%;top:-7px;width:14px;height:4px;transform:translateX(-50%);border-radius:999px;background:#d9ffd1}
    .stump.left{left:26px}
    .stump.center{left:50%;transform:translateX(-50%)}
    .stump.right{right:26px}
    .ball{position:absolute;top:50%;left:22px;width:16px;height:16px;border-radius:50%;background:radial-gradient(circle at 35% 35%,#f3ffd6,#b8f45c 48%,#547720 100%);box-shadow:0 0 0 6px rgba(184,244,92,.1),0 0 24px rgba(184,244,92,.45);transform:translateY(-50%);animation:ball-roll 1.45s cubic-bezier(.65,0,.35,1) infinite alternate}
    ...
    .pulse-dots i{width:9px;height:9px;border-radius:999px;background:#b8f45c;opacity:.35;animation:pulse 1s ease-in-out infinite}
