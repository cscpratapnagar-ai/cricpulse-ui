      ...
      @if (loading) {
        ...
        <div class="setup-card">
          <div class="step-head">
            <div>
              <b>01 · Toss result</b>
              <small>Select the winner and what they chose.</small>
            </div>
            <span>{{ match?.format || 'MATCH' }}</span>
          </div>

          <div class="teams">
            <button type="button" class="team-card" [class.selected]="winnerTeamId === match?.teamAId" (click)="winnerTeamId = match?.teamAId || ''">
              <span class="radio">{{ winnerTeamId === match?.teamAId ? '✓' : '' }}</span>
              <strong>{{ match?.teamAName || 'Team A' }}</strong>
              <small>Team A</small>
            </button>
            <div class="vs">VS</div>
            <button type="button" class="team-card" [class.selected]="winnerTeamId === match?.teamBId" (click)="winnerTeamId = match?.teamBId || ''">
          ...
          <div class="decisions">
            <button type="button" [class.active]="decision === 'BAT'" (click)="decision = 'BAT'">
              <span>🏏</span><b>BAT</b><small>Start with the bat</small>
            </button>
            <button type="button" [class.active]="decision === 'BOWL'" (click)="decision = 'BOWL'">
...
export class TossComponent {
  ...
  confirmToss(): void {
    ...
    this.saving = true;
    this.http.post<TossResponse>(`${this.api}/matches/${this.matchId}/toss`, {
