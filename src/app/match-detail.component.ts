      ...
      <section class="match-page">
        ...
        <section class="match-hero">
          ...
          <div class="hero-title-row">
            <div><h1>{{ match.name }}</h1><p>Everything for this fixture, from match setup to the final delivery.</p></div>
          ...
          <div class="fixture-board">
            <div class="team home"><div class="team-mark">{{ (match.teamAName || 'A').slice(0,1) }}</div><div><span>HOME SIDE</span><strong>{{ match.teamAName || 'Team A' }}</strong></div></div>
            ...
            <div class="team away"><div><span>AWAY SIDE</span><strong>{{ match.teamBName || 'Team B' }}</strong></div><div class="team-mark alt">{{ (match.teamBName || 'B').slice(0,1) }}</div></div>
          </div>
        </section>
...
@keyframes blink{50%{opacity:.35}}@keyframes shimmer{to{background-position:-200% 0}}@media(max-width:920px){.fixture-board{grid-template-columns:1fr 130px 1fr}.command-grid{grid-template-columns:1fr}.hero-title-row{align-items:flex-start;flex-direction:column}.hero-actions{width:100%}.lower-grid{grid-template-columns:1fr}}@media(max-width:680px){.match-page{padding:30px 18px 80px}.match-hero{padding:20px}.fixture-board{grid-template-columns:1fr;gap:14px}.fixture-core{order:2}.team.away{order:3;justify-content:flex-start;text-align:left}.team.away .team-mark{order:-1}.hero-actions{justify-content:space-between}.primary{flex:1;justify-content:center}.command-grid,.lower-grid{gap:12px}.panel{padding:18px}.metrics{grid-template-columns:1fr}.hero-title-row h1{font-size:32px;letter-spacing:-1.5px}}@media(prefers-reduced-motion:reduce){*,*:before,*:after{animation-duration:.01ms!important;transition-duration:.01ms!important}}`]
