  ...
  template: `
    <section class="card">
      ...
      <div class="connect-row">
        <input [(ngModel)]="inningsId" placeholder="Paste innings UUID" /><button
          (click)="connect()"
        >
          Connect
        </button>
      ...
      @if (score$ | async; as score) {
        @if (score) {
          ...
          <p class="waiting">Waiting for the first score update...</p>
        }
      }
    </section>
  `,
  styles: [
    `
      .card {
        ...
        border-radius: 22px;
        padding: 25px;
        background: linear-gradient(145deg, #10251eaa, #0b1b16dd);
      ...
      button:hover {
        ...
        box-shadow: 0 8px 18px #b8f45c30;
      }
      .score {
        font-size: 86px;
        font-weight: 850;
